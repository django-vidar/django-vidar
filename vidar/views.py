...
from vidar import app_settings, forms, helpers, interactor, oneoffs, storages, tasks, utils
...
class ChannelLivePlaylistsView(PermissionRequiredMixin, UseProviderObjectIdMatchingMixin, DetailView):
    ...
    def dispatch(self, request, *args, **kwargs):
        self.object = self.get_object()
        self.live_playlists = interactor.channel_playlists(self.object.provider_object_id, instance=self.object)
...
def video_sub_to_channel(request, pk):
    ...
    if not channel_provider_object_id:
        data = interactor.video_details(url=video.url, instance=video)
...
class VideoManageView(PermissionRequiredMixin, DetailView):
    ...
    def post(self, request, pk):
        ...
        if "refresh_thumbnail" in request.POST:
            dlp_output = interactor.video_details(self.object.url, quiet=True, instance=self.object)
